...
# Gets called each optimization step
def draw(c):
    c.background(1.0)
    c.fill(0, 0.2)
    c.stroke(0)
    ...
    for y in np.linspace(0, c.height, n + 2)[1:-1]:
        for x in np.linspace(0, c.width, n + 2)[1:-1]:
            c.push_matrix()
            c.translate(x, y)
            # optimization vars are created/initialized once and updated at each step
            # so "rot" will be zero at the beginning and then vary depending on loss
